	...
	frame = [
			[sg.Input('Enter your name :',
			 ...
			 sg.DropDown(class_list,
			 			 ...
			 			 size = (23,5))]]
	layout = [
			 [sg.Image('Logos\\AB.png',
			 			pad = (0,0))],
			 [sg.Frame('Enter your details : ',
			 			frame, size = (525,54))],
...
def phone_friend(data,i):
	if data[i][6] == 'A':
		...
		ans = data[i][4]	
		answer = 'The correct answer is C;'+ ans
	
	else:
		ans = data[i][5]	
		answer = 'The correct answer is;'+ ans
...
def create_lwindow(i):
	mixer.pause()
	sg.theme('Black')
	win_list = [1000,2000,3000,5000,
				10000,20000,40000,80000,
				160000,320000,640000,1250000,
				2500000,5000000,70000000,0]
	win_amt = win_list[i-1]
	
	layout = [[sg.Text ('Congratulations, your winning amount is Ruppees - ' + str(win_amt),
			   	  		text_color = '#FFCC00',
	  		   	  		font = 'Arial'),
			  sg.Button('QUIT', 
			 			key = '-QUIT-', 
...
def create_mwindow():
	...
	[sg.Button('A. '+ data[i][2]+ A_Space,
				...
				pad = (5,5),
				font = 'Consolas 15'),
	 sg.Button('B. '+ data[i][3]+ B_Space,
	 			key = 'B',
	 			expand_x = True, 
	 			button_color = button_color, 
	 			size = (button_size),
	 			font = 'Consolas 15')],
	[sg.Button('C. '+ data[i][4]+ C_Space,
				key = 'C',
				expand_x = True, 
				button_color = button_color, 
				size = (button_size), 
	...
	[sg.Image(r'Logos\50-50.png',
			  pad = image_pad,
			  key = '-50-',
			  enable_events = True),
	 sg.Image(r'Logos\audiencePole.png',
	 		  expand_x = True,
	 		  key = '-AP-',
	 		  enable_events = True),
	 sg.Image(r'Logos\phoneAFriend.png',
	 		  pad = image_pad,
	 		  key = '-PF-',
	 		  enable_events = True)],
	[sg.Image(r'Logos\center4.png',
				expand_x = True)],
	[sg.Text(str(i+1) + '. ' + data[i][1],key = '-QUE1-',
			 pad = (74,0), expand_x = True )],
	[sg.Text(key = '-QUE2-',pad = (98,0))],
	[sg.Text()],
	[sg.Frame(' Options : ', frame,pad = (70,0),
				element_justification = 'centre')]])
	image_col = sg.Column([[sg.Image("Wins\\picture0.png",
									 pad=(0,9), 
									 expand_x = True,
									 key = '-MONEY-')]])
